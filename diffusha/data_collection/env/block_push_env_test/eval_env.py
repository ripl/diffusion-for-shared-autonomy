...
from diffusha.data_collection.env.block_pushing import block_pushing_discontinuous
...
from diffusha.data_collection.env.block_push_env_test import tasks
try:
  from diffusha.data_collection.env.block_push_env_test import d4rl_utils  # pylint: disable=g-import-not-at-top
...
def get_env_name(task, shared_memory_eval, use_image_obs=False):
  ...
  if task in ['REACH', 'PUSH', 'INSERT', 'REACH_NORMALIZED', 'PUSH_NORMALIZED']:
    ...
    env_name = block_pushing_multimodal.build_env_name(
        task, shared_memory_eval, use_image_obs=use_image_obs)
  elif task in ['PUSH_MULTIMODAL_1BLOCK']:
    env_name = block_pushing_multimodal_1block.build_env_name(
      task, shared_memory_eval, use_image_obs=use_image_obs)
  elif task == 'PARTICLE':
