...
"""
    ...
    def reset(self, mask=None, seeds=None):
        self._assert_not_closed()
        if mask is None:
        ...
        if seeds is None:
            ...
            assert len(mask) == len(seeds)

        obs = []
        for m, remote, seed, last_o in zip(mask, self.remotes, seeds, self.last_obs):
            # import pdb; pdb.set_trace()
            if not m:
                if seed:
                    remote.send(("reset", int(seed)))
                else:
